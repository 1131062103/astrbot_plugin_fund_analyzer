...
为 AstrBot 提供专业的基金/股票数据分析功能。集成 **多智能体博弈分析**、实时行情、技术分析、量化指标、策略回测等丰富特性。

> 指令以两个独立的**指令组**提供：`基金 智能分析`、`股票 智能分析`（均是多智能体博弈分析）等，详见下方「命令一览」。

---

## ⚖️ 特色功能一：多智能体博弈分析（股票 智能分析）
...
> 💡 **使用方法**：发送 `股票 智能分析 600519` — 共 9 次 AI 对话，约 3-5 分钟出结果

---

## 🤖 特色功能二：多智能体博弈分析（基金 智能分析）
...
> 💡 **使用方法**：发送 `基金 智能分析 161226` 即可获取多智能体博弈分析报告
>
...
插件指令已拆分为**两个相互独立的指令组**：`基金` 与 `股票`，以及独立高级指令 `今日行情`（金/银贵金属）。

### 🟣 基金指令组（只含基金功能）

| 命令                         | 说明              | 示例                         |
| ---------------------------- | ----------------- | ---------------------------- |
| `基金 行情 [代码]`           | 查询基金实时行情  | `基金 行情 161226`           |
| `基金 技术分析 [代码]`       | 技术指标分析      | `基金 技术分析 161226`       |
| `基金 历史 [代码] [天数]`    | 查看历史行情      | `基金 历史 161226 20`        |
| `基金 搜索 关键词`           | 搜索 LOF 基金     | `基金 搜索 白银`             |
| `基金 设置 [代码]`           | 设置默认基金      | `基金 设置 161226`           |
| `基金 智能分析 [代码]`       | ⚖️ 多智能体博弈分析 | `基金 智能分析 161226`    |
| `基金 量化分析 [代码]`       | 专业量化绩效指标  | `基金 量化分析 161226`       |
| `基金 对比 代码1 代码2`      | 两只基金对比分析  | `基金 对比 161226 513100`    |
| `基金 帮助`                  | 显示基金帮助      | `基金 帮助`                  |
...
| 命令                         | 说明              | 示例                         |
| ---------------------------- | ----------------- | ---------------------------- |
| `股票 行情 <代码>`           | 查询 A 股实时行情 | `股票 行情 600519`           |
| `股票 搜索 关键词`           | 搜索 A 股股票     | `股票 搜索 茅台`             |
| `股票 智能分析 <代码>`       | ⚖️ 多智能体博弈分析 | `股票 智能分析 600519`    |
| `股票 技术分析 <代码>`       | 技术指标分析      | `股票 技术分析 600519`       |
| `股票 量化分析 <代码>`       | 专业量化绩效指标  | `股票 量化分析 600519`       |
| `股票 历史 <代码> [天数]`    | 查看历史行情      | `股票 历史 600519 20`        |
| `股票 对比 代码1 代码2`      | 两只 A 股对比分析 | `股票 对比 600519 000001`    |
| `股票 帮助`                  | 显示股票帮助      | `股票 帮助`                  |
